...
@app.route('/cards/')
#cli.command('all_cards')
def all_cards():
    #select all cards;
    # cards= Card.query.all() # print(stmt)
    # stmt = db.select(Card).where(Card.status == 'To Do')
    #----sort by group then title but descending---- #stmt = db.select(Card).order_by(Card.priority.desc(), Card.title)
    #----sort by group then title---- # stmt = db.select(Card).order_by(Card.priority, Card.title)
    #(db.or_(Card.status == 'To Do', Card.priority == 'High'))#id > 2)
    # (Card.status == 'Ongoing', Card.priority == 'High')
